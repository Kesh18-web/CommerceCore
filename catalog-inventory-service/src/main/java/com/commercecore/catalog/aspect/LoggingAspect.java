package com.commercecore.catalog.aspect;
...
public class LoggingAspect {
    ...
    @Before("execution(* com.commercecore.catalog.service.impl.*.*(..))")
    public void logBeforeMethod(JoinPoint joinPoint) {
        log.info("Starting method: " + joinPoint.getSignature().getName());
    }

    // Ghi log sau khi các method hoàn thành
    @AfterReturning(pointcut = "execution(* com.commercecore.catalog.service.impl.*.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        log.info("Completed method: " + joinPoint.getSignature().getName() + " with result: " + result);
    }

    // Ghi log khi có exception
    @AfterThrowing(pointcut = "execution(* com.commercecore.catalog.service.impl.*.*(..))", throwing = "error")
